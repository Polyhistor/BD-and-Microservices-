...
% CQRS. it's a nice pattern. for example using AVro file because it's write optimized and then we make a change in format, we use for example dataLabs, and dataLabs does the data handling. 

% One part is to bring the data into our platform to optimize for writing, and then we optimize that for the use case. This means that it's not in parquet all the time. If the use case say we need something more row-based and need more aggregation, then we optimize is for the use-case. It really depends on what the customer needs and not our standard. 

% at one time, it's good for the customer, on the other hand it's hard for us. because we have to understand the requirements of the customer. because the business user doesn't know what is optimized, but she says I wanna do some aggregation, but you as the data engineer have to understand that it's the aggregation of the different columns, maybe filter some critera and decide the best format. We try to do some decision matrix to understand use what for what data cosnumer. 

% At the end, the user is more happy about power BI query taking ages. we got some very old Java applications, you put the button on Friday so you have the results on Monday, even data is not much because of data format. 

% anti-corruption layer: it really depends on use case. the biggest problem of the data science, is that they say they need all the information. Because we have to know the question, you don't know everything. With this layer, you will have the chance from the governance perspective, to say that what he/she can see. and what is the purpose. In the finance and insurance sector, the purpose is really important, specially in germany. Because the data security part is very important, and for example we had a use case and security team said we can't provide it because it's too sensitive. 

% this gives the chance for optimization from user and from the other hand, it provides with governance. 

% security and privacy :

% BFF : the point is that this pattern has conflict with logging. IF you increase the number of API gateway, if we use special BFFs for each data consumer class, and another issue is controling of what comes in and what goes out, makes it really difficult. 

% from a security persective, things will be really hard, and they will need more people to understand what is happening on different backends for fronteds.

% from my perspective, it's just an increased complexity. at the end, the operation will have a happy life, if you can control your platform easy. 

% if you bring this into production, our life is gonna be hard, because we have a lot of work to monitor and makes our life really hard. 

% the amindstrator is lazy, and he/she does not want to handle gateways. 

% veracity: 

% absolutely agree with these patterns. it's better that you don't bring a lot of transformation in one place. if you have bad results you will never know how that happened. 

% for us, data quality is becoming more and more important. because based on our data, we make a lot of decision. because based on that data we decide claims. 

% Sometimes in our old world it's not clear how did we come to this result and if the result was right or not. 

% so the results are not clear. so the first pattern is perfect, and you know at each step what is the issue, and what happened. 

% and if you have data quality metrics at each point, you can come back and look at your pipe and see how data quality decreased. you can then understand what happened and why that happened.
