...
people don't really follow design patterns, they decise their own thing 

we use api gateway for conversion, when we created a new ETL process we constalty feed data to API Gateway to different regions across the world. 

event driven data is on our plan. we want to store the source data and wants its landing in the S3, we will trigger the lamda function to start the ETL process.

this is diffuclt because we have to get the configuration from the third-party. we like to go even-driven fully. 

configurations will be stored in DynamoDB and decide what should be done for what.


. There are known reasons for not applying microservice patterns in BD systems. For example, the CQRS pattern mentioned in Section V.D.1. to separate readers from writers would increase the overall data volume and stress the storage media. This even worsens in the Bigdata systems due to the large volume of data in these systems. Thus, the direct adaptation of the CQRS pattern in the BD system might not perform well. In addition, the microservice architecture itself has a few general overheads as well. For example, it is well known that microservice architecture would incur high operation overhead, and the authors failed to address any such issues.
